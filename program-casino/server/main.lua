...
AddEventHandler("startblackjack", function()
    ...
    local player1
    local player2
    TriggerClientEvent('esx:showAdvancedNotification', s, 'Oyuna Basla', 'Masayı Ac', 'Hazardzistow ~g~---', 'CHAR_BANK_MAZE', 9)
end)
...
AddEventHandler('program-casino:craftingCoktails', function(itemValue)
    ...
    TriggerClientEvent('esx:showNotification', _source, 'Devam eden çeşitli bileşenlerin karıştırılması!')
    ...
    if _itemValue == 'jagerbomb' then
        SetTimeout(10000, function()        
            ...
            if alephQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .."energy drinka " .. '~w~')
            elseif bethQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .."jagera " .. '~w~')
            else
                local chanceToMiss = math.random(100)
                if chanceToMiss <= Config.MissCraft then
                    TriggerClientEvent('esx:showNotification', _source,'Ortalama olarak, bu karışım çıktı ...')
                    xPlayer.removeInventoryItem('energy', 2)
                    xPlayer.removeInventoryItem('jager', 2)
                else
                    TriggerClientEvent('esx:showNotification', _source, "Komple karıştırma " .. 'jagerbomba' .. ' ~w~!')
                    xPlayer.removeInventoryItem('energy', 2)
                    xPlayer.removeInventoryItem('jager', 2)
    ...
    if _itemValue == 'golem' then
        SetTimeout(10000, function()        
            ...
            if alephQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'lemoniada' .. '~w~')
            elseif bethQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. "voodki" .. '~w~')
            elseif gimelQuantity < 1 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'lód' .. '~w~')
            else
                local chanceToMiss = math.random(100)
                if chanceToMiss <= Config.MissCraft then
    ...
    if _itemValue == 'whiskycoca' then
        SetTimeout(10000, function()        
            ...
            if alephQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'cola' .. '~w~')
            elseif bethQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. "whisky" .. '~w~')
            else
                local chanceToMiss = math.random(100)
                if chanceToMiss <= Config.MissCraft then
                    TriggerClientEvent('esx:showNotification', _source,'Srednio wyszla ta mieszanka ...')
    ...
    if _itemValue == 'rhumcoca' then
        SetTimeout(10000, function()        
            ...
            if alephQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'woda' .. '~w~')
            elseif bethQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'rum' .. '~w~')
            else
                local chanceToMiss = math.random(100)
                if chanceToMiss <= Config.MissCraft then
                    TriggerClientEvent('esx:showNotification', _source,'Srednio wyszla ta mieszanka ...')
    ...
    if _itemValue == 'vodkafruit' then
        SetTimeout(10000, function()        
            ...
            if alephQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " ..'sok owocowy' .. '~w~')
            elseif bethQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. "voodki" .. '~w~')
            elseif gimelQuantity < 1 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'lód' .. '~w~')
    ...
    if _itemValue == 'teqpaf' then
        SetTimeout(10000, function()        
            ...
            if alephQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'lemoniada' .. '~w~')
            elseif bethQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. "tequili" .. '~w~')
            else
                local chanceToMiss = math.random(100)
                if chanceToMiss <= Config.MissCraft then
                    TriggerClientEvent('esx:showNotification', _source,'Srednio wyszla ta mieszanka ...')
    ...
    if _itemValue == 'mojito' then
        SetTimeout(10000, function()        
            ...
            elseif gimelQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'Mięta' .. '~w~')
    ...
    if _itemValue == 'mixapero' then
        SetTimeout(10000, function()        
            ...
            if alephQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'Orzeszki' .. '~w~')
            elseif bethQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'Orzeszki ziemne' .. '~w~')
            elseif gimelQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'Pistacje' .. '~w~')
            elseif daletQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'Czipsy' .. '~w~')
            else
                local chanceToMiss = math.random(100)
                if chanceToMiss <= Config.MissCraft then
                    TriggerClientEvent('esx:showNotification', _source,'Srednio wyszla ta mieszanka ...')
    ...
    if _itemValue == 'jagercerbere' then
        SetTimeout(10000, function()        
            ...
            if alephQuantity < 1 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. 'jagerbomba' .. '~w~')
            elseif bethQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. "voodki" .. '~w~')
            elseif gimelQuantity < 2 then
                TriggerClientEvent('esx:showNotification', _source, "Yeterli Değil " .. "tequili" .. '~w~')
...
AddEventHandler('program-casino:getStockItem', function(itemName, count)
  ...
  TriggerEvent('esx_addoninventory:getSharedInventory', 'society_casino', function(inventory)
    ...
    if item.count >= count then
      inventory.removeItem(itemName, count)
      xPlayer.addInventoryItem(itemName, count)
      TriggerClientEvent('esx:showNotification', xPlayer.source, "Dolaptan Aldın " .. count .. 'x ' .. item.label)
    else
      TriggerClientEvent('esx:showNotification', xPlayer.source, "Kötü Miktar")
    end
