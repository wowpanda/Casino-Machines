...
local blips = {
	{title="Diamond Casino", colour=29, id=33, x = 930.17, y = 41.8, z = 37.3}
}
...
Citizen.CreateThread(function()
	...
	-------------------------------------------------------Poczatek TP
positions = {
    {{924.26, 46.94, 80.00, 0}, {1090.00, 207.00, -49.9, 0}},} --
	...
	while true do
		...
		for i,location in ipairs(positions) do
			...
			if CheckPos(playerLoc.x, playerLoc.y, playerLoc.z, loc1.x, loc1.y, loc1.z, 2) then
				alert("Giriş İcin E Tusuna Basınız")

				if IsControlJustReleased(1, 38) then
					if IsPedInAnyVehicle(player, true) then
						PlayerData = ESX.GetPlayerData()
						local elements = {
							{label = "Normal Bilet",  value = 'classicTicket'},
							{label = "Altın Bilet",  value = 'goldTicket'},
							{label = "Vip Bilet", value = "platiniumTicket"},
							{label = "İsci Girisi", value = "worker"}
						}
						...
						PlayerData = ESX.GetPlayerData()
						local elements = {
							{label = "Normal Bilet ["..Config.classicPrice.."$]",  value = 'classicTicket'},
							{label = "Altın Bilet ["..Config.goldenPrice.."$]",  value = 'goldTicket'},
							{label = "Vip Bilet ["..Config.platiniumPrice.."$]", value = "platiniumTicket"},
							{label = "İsci Girisi", value = "worker"}
						}
...
AddEventHandler('program-casino:hasEnteredMarker', function(zone)
	if zone =='CasinoActions' then
		...
		CurrentActionMsg  = "~r~[E] ~b~Kumarhane Yönetimi Menüsü Aç"
		CurrentActionData = {}
	elseif zone == "CasinoCloak" then
		CurrentAction     = 'CasinoCloak'
		CurrentActionMsg  = "~r~[E] ~p~Kumarhaneyi Degistirme"
		CurrentActionData = {}
	elseif zone == "CasinoFridge" then
		CurrentAction     = 'CasinoFridge'
		CurrentActionMsg  = "~r~[E] ~p~Buzdolabı"
		CurrentActionData = {}
...
AddEventHandler('program-casino:openTicketMenuClient', function(worker)
	...
	if goldTicket then
		table.insert(elements, {label = "Altın Bileti Göster", value = 'showGolden'})
	end
	if platiniumTicket then
		table.insert(elements, {label = "Vip Bileti Göster", value = 'showPlatinium'})
	end
	if classicTicket then
		table.insert(elements, {label = "Bileti Göster", value = 'showClassic'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pokazTicket', {
		...
		elseif data.current.value == 'showGolden' then
			TriggerServerEvent('program-casino:sendTicket', "Müsterinin Altın Biletİ Var", worker)
		elseif data.current.value == 'showPlatinium' then
			TriggerServerEvent('program-casino:sendTicket', "Müsterinin Vip Bileti Var", worker)
		elseif data.current.value == 'showClassic' then
			TriggerServerEvent('program-casino:sendTicket', "Müsterinin Normal Bileti Var", worker)
		end
...
function OpenCasinoActionsMenu()
	PlayerData = ESX.GetPlayerData()
	local elements = {
		{label = "Bilet İsteyin",  value = 'ticket'}
	}
	if closeStock == true then
		table.insert(elements, {label = "Silah Dolabı", value = 'weaponArmory'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'casino_mgmt', {
		...
		if data.current.value == 'ticket' then
			...
			if closestPlayer ~= -1 and closestDistance <= 3.0 then
				...
				ESX.ShowNotification('~r~Etrafında Kimse Yok')
			end
		...
		function(data, menu)
		  ...
		  ESX.UI.Menu.Open(
			...
			function(data2, menu2)
			  ...
			  if count == nil then
				ESX.ShowNotification("Kötü Miktar")
			  else
				menu2.close()
				menu.close()
				OpenGetFridgeStocksMenu()
...
function OpenVaultMenu()
	...
	local elements = {
		{label = "Silahı Al", value = 'get_weapon'},
		{label = "Silahı Bırak", value = 'put_weapon'},
		{label = "Esyaları Al", value = 'get_stock'},
		{label = "Esyaları Koy", value = 'put_stock'}
	}
